...
2. **Raster data**: Binary raster files (`public/data/*.bin` + `.meta.json`) provide monthly precipitation and ET0. Coordinates reprojected from WGS84 into each raster's own CRS via proj4 (`CRS_DEFS` in `rasterData.ts` — a raster whose `meta.crs` is not listed there throws on load). Used by gemuese_obst, weinbau, gruenflaechen, naturrasen, golf, tennen.
   - **The products are prescribed, not interchangeable.** Kapitel 4.1.3 names HYRAS for precipitation (`preciphyras_*`, EPSG:3035) and FAO-56 grass reference for ET₀ (`et0fao_*`, EPSG:31467); the Kapitel 4.2/4.3 tables are valid only for them. The app previously used `multi_annual/evapo_p`, which is **AMBAV/Haude — a different evaporation model** — putting the ΔKWB correction in `gemuese_obst` on a non-conformant basis. Rebuild only via `scripts/build_raster_nc.py`; `scripts/deploy.sh` aborts if `meta.source` names the wrong product.
   - Validation against the Merkblatt's own Potsdam reference (`refKwb` in `soilConstants.ts`, Mar–Oct): total absolute deviation **27 mm → 7 mm** after the switch, four months exact, none off by more than 2 mm.
...
- **Third-party licenses**: the root `THIRD-PARTY-LICENSES.txt` bundles the license texts of every dependency shipped in the browser bundle and is linked from the `?`-Dialog (`AboutPage.tsx`). **It is generated — do not hand-edit.** Whenever a runtime dependency is added/removed/upgraded, re-run `node app/scripts/gen-third-party-licenses.cjs` and commit the regenerated file. The generator's `pkgs` list must be kept in sync with the shipped runtime deps; a new dep whose `license` isn't detectable from a bundled LICENSE file needs a fallback entry there.
  - **On every dependency change (add/remove/upgrade), check whether the user-visible license list in `AboutPage.tsx` needs updating** — it hand-lists selected libraries with their licenses (grouped by license type). Adding/removing a shipped runtime dep, or an upgrade that changes a library's license, must be reflected there in addition to regenerating `THIRD-PARTY-LICENSES.txt`. Dev-only deps that never ship in the browser bundle don't belong in either.
