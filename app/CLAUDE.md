...
- `components/tour/` — **guided walk-through** (Rundgang) across pages, in **two variants** (`tourVariant: "demo" | "empty"`). `tourSteps.ts` exports `demoSteps`, `emptySteps`, `tourStepsFor(variant)`, and a shared `fieldStep()` (add/edit fields — used read-only by demo, create-mode by empty). Each `TourStep` names a `data-tour` target, German title/body, and `advanceOn: "route" | "state" | "button"`:
    - **demo** (Beispieldaten geladen): Flächen → *Flächen anlegen/bearbeiten* → Szenarien-Tab → Szenario öffnen → Zuweisung → Zusammenfassung. Mostly `route`/`button` advance; stays in context, doesn't open forms.
    - **empty** (no data): Betrieb benennen → Fläche anlegen → *weitere Flächen? (Weiter)* → Szenarien-Tab → Szenario anlegen → Fläche zuweisen → Nutzung festlegen (Zuweisung öffnen) → Nutzung wählen & speichern → *weitere Zuweisungen? (Weiter)* → Zusammenfassung. Uses `advanceOn:"state"` with a `done(ctx)` predicate (`ctx = {farm, projects, demoProjectId, pathname}`) — the step auto-advances when the real action happened (name set, field added, project created → on `/projects/:id`, an assignment created, the AssignmentPage opened, then the assignment gained a `module`). The two "weitere …?" steps are `button`-advance loops: the user may repeat the create action (more fields / more assignments) and clicks **Weiter** when done. **Store-vs-local caveat:** on the AssignmentPage the module is local state until *saved*, so `hasAssignedModule` only fires on save — the `configure-assignment` step therefore targets the bottom **`assignment-save`** button (after picking a module the page auto-scrolls down, so the spotlight sits by the active inputs) and advances when the assignment is saved. The preceding `open-usage` step targets the `assignment-row` on the project page and advances when the AssignmentPage route opens.
  `useTourTarget.ts` resolves `[data-tour="…"]`, rAF-polls until it mounts after a route change, scrolls it into view, tracks its rect on scroll/resize (`ResizeObserver`). `TourOverlay.tsx` picks the step array via `tourStepsFor(tourVariant)`, renders a box-shadow "spotlight" (target stays clickable — dimmer is `pointer-events:none`) plus a `@floating-ui/react` tooltip (`offset`/`flip`/`shift`/`arrow`); `route` steps advance when the URL matches the next step's route, `state` steps when `done(ctx)` flips true, `button` steps via **Weiter/Fertig**. Runtime state (`tourActive`/`tourVariant`/`tourStep` + `startTour(variant)`/`nextTourStep`/`endTour`) lives in `useAppStore`; the persisted `dwa_tour_completed` flag (`useLocalStore`) gates the demo offer. Targets carry `data-tour="farm-name|farm-fields|add-field|nav-scenarios|project-row|add-scenario|assignment-row|add-assignment|assignment-save|project-summary"`. Add-controls that toggle to a form (`add-field`, `add-scenario`, `add-assignment`) put the `data-tour` on a **persistent wrapper** around button+form, so the spotlight keeps tracking while the form is open. **Entry points:** the FarmPage offer ("🧭 Rundgang starten?", when `demoProject && !dwa_tour_completed && !tourActive`, **Später** sets the flag) starts the **demo** tour; the `OnboardingOverlay` button starts **demo** if a demo exists else **empty** (guided creation). `Escape`/Überspringen end the tour and set completed.
