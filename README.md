...
Android Studio is only an IDE around the command-line tools; the APK can be
built headlessly. Needs JDK 21 (`java -version`) and about 6 GB.

```bash
# once: SDK command-line tools, then platform + build-tools for API 36
...
# each build
cd app && yarn build --mode android && yarn cap:sync:android
cd android && ./gradlew assembleDebug     # or bundleRelease for Play
```
...
Two obstacles on a machine that has never run Android Studio:
...
- **`app/android/gradle/gradle-daemon-jvm.properties`** is generated by Android
  Studio and pins `toolchainVendor=jetbrains`. Gradle then refuses to use a
  non-JetBrains JDK 21 and cannot download one for Linux/x86_64. Drop the
  `toolchainVendor` line (keep `toolchainVersion=21`) to build with OpenJDK.
